...
`prompts/host.md`, as JSON segments (intro / story×3 / headline×10–12 tagged with the digest
heading / outro) ·
...
`#speechCaption` style, plus the layout timeline (each card's span, the PIP
window) · `04-cards.sh` one stage card per story (theme title + its top 3
tweets) and per headline (the line, big, over the tweet it came from),
initial-letter avatars, screenshotted by the cached Playwright Chromium
(`render_cards.mjs`, no network) · `05-render.sh` comp: clawd full-frame for
intro/outro, bottom-right PIP with the card on the stage during the stories
and the headline run, headline ticker, burned captions → H.264 mp4. Headlines
are separated by ElevenLabs `<break>` tags for the read-pause-read rhythm. `scripts/fetch-rig.sh` re-vendors the rig; nothing here
